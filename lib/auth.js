...
function sessionKey(){if(process.env.SESSION_SECRET)return process.env.SESSION_SECRET;if(process.env.VERCEL)throw Error('Session signing key unavailable');return localSessionKey}
function sign(v){return createHmac('sha256',sessionKey()).update(v).digest('hex')}
